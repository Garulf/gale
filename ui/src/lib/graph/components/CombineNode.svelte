<script>
  ...
  let outputLabel = $derived(config.type === 'mix' ? `${config.mode} of inputs` : 'follows input');
</script>

<div class="node duty-kind" class:sel={selected} class:warned={warnings.length > 0} data-node-id={id}>
  <h4>
    <span class="kind"></span>
    <span class="title"><span class="name">{data.combine.id}<WarningBadge messages={warnings} /></span><small>combine · {config.type}</small></span>
  </h4>
  <div class="rows">
